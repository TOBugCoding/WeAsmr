...
Item{
    ...
    Row{
		...
		Item{
			id: rightContentItem
            //这里width是链式绑定，只会生效依次，需要binding去维护
            width: left_btn_list.visible ? (rootRow.width - left_btn_list.width) : rootRow.width
            height: parent.height
            //Behavior on width { NumberAnimation { duration: 200; easing.type: Easing.OutQuad } }
            Binding { target: rightContentItem; property: "width"; value: left_btn_list.visible ? (rootRow.width - left_btn_list.width) : rootRow.width }
			Rectangle{
